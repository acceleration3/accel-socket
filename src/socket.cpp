...
namespace accel
{
	...
	ip_address_v4::ip_address_v4(std::uint32_t numeric) : 
		m_value(numeric) {}
	...
	ip_address_v6::ip_address_v6(const std::array<std::uint16_t, 8>& shorts) : 
		m_values(shorts) {}
	...
	std::size_t socket::receive(std::uint8_t* data, int length)
	{
		std::size_t data_length = recv(m_impl->sock, reinterpret_cast<char*>(data), length, 0);
		if (data_length < 0)
			throw socket_exception();
		return data_length;
	}

	std::size_t socket::send(const std::uint8_t* data, int length)
	{
		std::size_t data_length = ::send(m_impl->sock, reinterpret_cast<const char*>(data), length, 0);
		if (data_length < 0)
			throw socket_exception();
		return data_length;
	}

	std::size_t socket::receive_from(endpoint& ep, std::uint8_t* data, int length)
	{
		if (m_impl->family == AF_INET)
		{
			struct sockaddr_in address;
			socklen_t from_length = sizeof(address);
			std::size_t data_length = recvfrom(m_impl->sock, reinterpret_cast<char*>(data), length, 0, reinterpret_cast<sockaddr*>(&address), &from_length);
			if (data_length < 0)
				throw socket_exception();
			ep.m_ip = ip_address_v4(address.sin_addr.S_un.S_addr);
			ep.m_port = address.sin_port;
			return data_length;
		}
		else
		{
			...
			ep.m_port = address.sin6_port;
			return data_length;
		}
	}

	std::size_t socket::send_to(const endpoint& ep, const std::uint8_t* data, int length)
	{
		if (m_impl->family == AF_INET)
		{
			...
			socklen_t from_length = sizeof(address);
			std::size_t data_length = sendto(m_impl->sock, reinterpret_cast<const char*>(data), length, 0, reinterpret_cast<const sockaddr*>(&address), from_length);
			if (data_length < 0)
				throw socket_exception();
			return data_length;
		}
